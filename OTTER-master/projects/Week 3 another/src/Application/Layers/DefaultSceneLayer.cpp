...
void DefaultSceneLayer::_CreateScene()
{
	...
	if (loadScene && std::filesystem::exists("scene.json")) {
		...
		ShaderProgram::Sptr nodiffShader = ResourceManager::CreateAsset<ShaderProgram>(std::unordered_map<ShaderPartType, std::string > {
			{ShaderPartType::Vertex, "shaders/vertex_shaders/basic.glsl"},
			{ShaderPartType::Fragment, "shaders/fragment_shaders/textured_specular.glsl"}
		});

		ShaderProgram::Sptr nospecShader = ResourceManager::CreateAsset<ShaderProgram>(std::unordered_map<ShaderPartType, std::string>{
			{ShaderPartType::Vertex, "shaders/vertex_shaders/basic.glsl"},
			{ShaderPartType::Fragment, "shaders/fragment_shaders/frag_blinn_phong_textured_nospec.glsl"}
		});

		ShaderProgram::Sptr nolightShader = ResourceManager::CreateAsset<ShaderProgram>(std::unordered_map<ShaderPartType, std::string>{
			{ShaderPartType::Vertex, "shaders/vertex_shaders/basic.glsl"},
			{ ShaderPartType::Fragment, "shaders/fragment_shaders/frag_blinn_phong_textured_nolight.glsl" }
		});

		// Load in the meshes
		MeshResource::Sptr monkeyMesh = ResourceManager::CreateAsset<MeshResource>("Monkey.obj");
		MeshResource::Sptr binMesh = ResourceManager::CreateAsset<MeshResource>("BigBen.obj");
		MeshResource::Sptr trashMesh = ResourceManager::CreateAsset<MeshResource>("trashywalk.obj");
		MeshResource::Sptr fountainMesh = ResourceManager::CreateAsset<MeshResource>("FountainTall.obj");
		...
		Texture2D::Sptr TrashTex = ResourceManager::CreateAsset<Texture2D>("textures/trashyTEX.png");
		Texture2D::Sptr FountainTex = ResourceManager::CreateAsset<Texture2D>("textures/fountain.jpg");
		...
		Texture2D::Sptr grey = ResourceManager::CreateAsset<Texture2D>("textures/grey.jpg");
		...
		Texture1D::Sptr toonLut2 = ResourceManager::CreateAsset<Texture1D>("luts/toon-1D2.png");
		toonLut2->SetWrap(WrapMode::ClampToEdge);

		// Here we'll load in the cubemap, as well as a special shader to handle drawing the skybox
		TextureCube::Sptr testCubemap = ResourceManager::CreateAsset<TextureCube>("cubemaps/yellow/yellow.png");
		...
		Texture3D::Sptr lutCool = ResourceManager::CreateAsset<Texture3D>("luts/coolcar.CUBE");

		Texture3D::Sptr lutWarm = ResourceManager::CreateAsset<Texture3D>("luts/warmcar.cube");

		Texture3D::Sptr lutCustom = ResourceManager::CreateAsset<Texture3D>("luts/bwcar.CUBE");

		Texture3D::Sptr lutShrooms = ResourceManager::CreateAsset<Texture3D>("luts/shrooms.cube");
		...
		Material::Sptr noLightMat = ResourceManager::CreateAsset<Material>(nolightShader);
		{
			noLightMat->Name = "No Light";
		}

		Material::Sptr noDiffMat = ResourceManager::CreateAsset<Material>(nodiffShader);
		{
			noDiffMat->Name = "No Diffuse";
			noDiffMat->Set("u_Material.Diffuse", grey);
			noDiffMat->Set("u_Material.Shininess", 5.0f);
		}

		Material::Sptr noSpecTrashy = ResourceManager::CreateAsset<Material>(nospecShader);
		{
			noSpecTrashy->Name = "No spec trashy";
			noSpecTrashy->Set("u_Material.Diffuse", TrashTex);
		}

		Material::Sptr noSpecBin = ResourceManager::CreateAsset<Material>(nospecShader);
		{
			noSpecBin->Name = "No spec bin";
			noSpecBin->Set("u_Material.Diffuse", BinTex);
		}

		Material::Sptr noSpecFountain = ResourceManager::CreateAsset<Material>(nospecShader);
		{
			noSpecFountain->Name = "No spec Fountain";
			noSpecFountain->Set("u_Material.Diffuse", FountainTex);
		}

		Material::Sptr noSpecStatue = ResourceManager::CreateAsset<Material>(nospecShader);
		{
			noSpecStatue->Name = "No spec Statue";
			noSpecStatue->Set("u_Material.Diffuse", StatueTex);
		}

		Material::Sptr noSpecFloor = ResourceManager::CreateAsset<Material>(nospecShader);
		{
			noSpecFloor->Name = "No spec floor";
			noSpecFloor->Set("u_Material.Diffuse", floorTex);
		}

		Material::Sptr floorMat = ResourceManager::CreateAsset<Material>(basicShader);
		{
			floorMat->Name = "Floor";
			floorMat->Set("u_Material.Diffuse", floorTex);
			floorMat->Set("u_Material.Shininess", 0.1f);
		}

		Material::Sptr trashyMat = ResourceManager::CreateAsset<Material>(basicShader);
		{
			trashyMat->Name = "Trashy";
			trashyMat->Set("u_Material.Diffuse", TrashTex);
			trashyMat->Set("u_Material.Shininess", 0.1f);
		}

		Material::Sptr binMat = ResourceManager::CreateAsset<Material>(basicShader);
		{
			binMat->Name = "Bin";
			binMat->Set("u_Material.Diffuse", BinTex);
			binMat->Set("u_Material.Shininess", 0.1f);
		}

		Material::Sptr fountainMat = ResourceManager::CreateAsset<Material>(basicShader);
		{
			fountainMat->Name = "Fountain Mat";
			fountainMat->Set("u_Material.Diffuse", FountainTex);
			fountainMat->Set("u_Material.Shininess", 0.1f);
		}

		Material::Sptr statueMat = ResourceManager::CreateAsset<Material>(basicShader);
		{
			statueMat->Name = "Statue";
			statueMat->Set("u_Material.Diffuse", StatueTex);
			statueMat->Set("u_Material.Shininess", 0.1f);
		}
		
		Material::Sptr floorToon = ResourceManager::CreateAsset<Material>(toonShader);
		{
			floorToon->Name = "Floor Toon";
			floorToon->Set("u_Material.Diffuse", floorTex);
			floorToon->Set("s_ToonTerm", toonLut2);
			floorToon->Set("u_Material.Shininess", 1.0f);
			floorToon->Set("u.Material.Steps", 3);
		}

		Material::Sptr trashytoonMat2 = ResourceManager::CreateAsset<Material>(toonShader);
		{
			trashytoonMat2->Name = "Trashy Toon";
			trashytoonMat2->Set("u_Material.Diffuse", TrashTex);
			trashytoonMat2->Set("s_ToonTerm", toonLut2);
			trashytoonMat2->Set("u_Material.Shininess", 1.0f);
			trashytoonMat2->Set("u.Material.Steps", 3);
		}

		Material::Sptr bintoonMat2 = ResourceManager::CreateAsset<Material>(toonShader);
		{
			bintoonMat2->Name = "Bin Toon";
			bintoonMat2->Set("u_Material.Diffuse", BinTex);
			bintoonMat2->Set("s_ToonTerm", toonLut2);
			bintoonMat2->Set("u_Material.Shininess", 1.0f);
			bintoonMat2->Set("u.Material.Steps", 3);
		}

		Material::Sptr fountaintoonMat = ResourceManager::CreateAsset<Material>(toonShader);
		{
			fountaintoonMat->Name = "Fountain Toon";
			fountaintoonMat->Set("u_Material.Diffuse", FountainTex);
			fountaintoonMat->Set("s_ToonTerm", toonLut2);
			fountaintoonMat->Set("u_Material.Shininess", 1.0f);
			fountaintoonMat->Set("u_Material.Steps", 3);
		}

		Material::Sptr statuetoonMat = ResourceManager::CreateAsset<Material>(toonShader);
		{
			statuetoonMat->Name = "Statue Toon";
			statuetoonMat->Set("u_Material.Diffuse", StatueTex);
			statuetoonMat->Set("s_ToonTerm", toonLut2);
			statuetoonMat->Set("u_Material.Shininess", 1.0f);
			statuetoonMat->Set("u_Material.Setps", 3);
		}
		...
		{
			camera->SetPostion({ -5, -3.5, 3.5 });
			//camera->LookAt(glm::vec3(0.0f));
			camera->SetRotation(glm::vec3(76.5f, 0.0f, -86.0f));
		...
		GameObject::Sptr plane = scene->CreateGameObject("Plane");
		{
			...
			renderer->SetMaterial(noDiffMat);
		...
		GameObject::Sptr Trashy = scene->CreateGameObject("Trashy");
		{
			Trashy->SetPostion(glm::vec3(-0.43f, -3.79f, 0.0f));
			Trashy->SetRotation(glm::vec3(90.0f, 0.0f, 0.0f));

			RenderComponent::Sptr renderer = Trashy->Add<RenderComponent>();
			renderer->SetMesh(trashMesh);
			renderer->SetMaterial(noDiffMat);
 
			RotatingBehaviour::Sptr behaviour = Trashy->Add<RotatingBehaviour>();
			behaviour->RotationSpeed = glm::vec3(0.0f, 0.0f, -90.0f);
		}
		
		GameObject::Sptr BigBin = scene->CreateGameObject("Big Ben");
		{
			BigBin->SetPostion(glm::vec3(1.5f, 1.5f, 0.0f));
			BigBin->SetRotation(glm::vec3(90.0f, 0.0f, -137.0f));

			RenderComponent::Sptr renderer = BigBin->Add<RenderComponent>();
			renderer->SetMesh(binMesh);
			renderer->SetMaterial(noDiffMat);
		}

		GameObject::Sptr Fountain = scene->CreateGameObject("Tall Fountain");
		{
			Fountain->SetPostion(glm::vec3(6.0f, -5.0f, 0.0f));
			Fountain->SetRotation(glm::vec3(90.0f, 0.0f, 0.0f));
			Fountain->SetScale(glm::vec3(2.0f));
			
			RenderComponent::Sptr renderer = Fountain->Add<RenderComponent>();
			renderer->SetMesh(fountainMesh);
			renderer->SetMaterial(noDiffMat);
		}

		GameObject::Sptr statue = scene->CreateGameObject("Statue");
		{
			statue->SetPostion(glm::vec3(1.0f, -10.0f, 0.0f));
			statue->SetRotation(glm::vec3(90.0f, 0.0f, -150.0f));
			statue->SetScale(glm::vec3(1.0f));

			RenderComponent::Sptr renderer = statue->Add<RenderComponent>();
		...
		*/
		...
		GameObject::Sptr particles = scene->CreateGameObject("Particles");
		{
			...
			particleManager->AddEmitter(glm::vec3(6.0f,-5.0f,5.0f), glm::vec3(0.0f, -1.0f, 10.0f), 10.0f, glm::vec4(0.0f, 0.0f, 1.0f, 1.0f)); 
		}
